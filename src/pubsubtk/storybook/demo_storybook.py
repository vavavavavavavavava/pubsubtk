...
def label_primary(ctx):
    ...
    color = ctx.knob("color", str, "black", desc="文字色", 
...
def text_multiline(ctx: StoryContext):
    """複数行テキストエリア"""
    content = ctx.knob("content", str, 
                      "This is a multiline\ntext area example.\n\nYou can edit this content!",
                      desc="テキスト内容", multiline=True)
...
def ttk_button_themed(ctx: StoryContext):
    ...
    state = ctx.knob("state", str, "normal", desc="状態", 
                    choices=["normal", "disabled", "pressed"])
    width = ctx.knob("width", int, 15, desc="幅", range_=(10, 30))
    
    btn = ttk.Button(
        ctx.parent, text=text.value, width=width.value
    )
...
def ttk_progress_bar(ctx: StoryContext):
    ...
    mode = ctx.knob("mode", str, "determinate", desc="モード",
